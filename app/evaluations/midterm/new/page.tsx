...
const NewMidTermEvaluationPage: React.FC = () => {
  const { user } = useAuth();
  ...
  const [professionalIntegration, setProfessionalIntegration] = useState<number>(1);
  ...
  const [globalAppreciation, setGlobalAppreciation] = useState<number>(1);
  const [managerComment, setManagerComment] = useState<string>('');
  const [availableEmployees, setAvailableEmployees] = useState<any[]>([]); // Remplacez `any` par le type approprié pour vos employés


  // http://localhost:7000/staff/
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      if (user?.departmentId) {
        try {
          const response = await fetch(`http://localhost:7000/staff/All`);
          if (!response.ok) {
  ...
  const updateCompetency = (field: keyof Competencies, value: number) => {
    setCompetencies(prev => ({ ...prev, [field]: value }));
  };

  const handleSave = async (status: 'EN_COURS' | 'TERMINEE') => {
    ...
    try {
      const response = await fetch('http://localhost:7000/evaluation', {
        method: 'POST',
        headers: {
